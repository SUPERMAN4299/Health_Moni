...
# ---------------- BT scanning ---------------- #
''''
btcl_path = r"C:\Userial_ports\HP\Downloads\bluetoothcl\BluetoothCL.exe"
...
def scan_bt():
    ...
    for line in result.stdout.splitlines():
        ...
        if match:
            mac_addresses.append(match.group(1).lower())
...
def connection():
    ...
    #device_connected = scan_bt() == stored_mac_enc
    if stored_mac_enc in scan_bt():
...
def read_sensor_data():
    ...
    global HEART_DATA, GSR_DATA, TEMP_DATA, SPO2
...
LLM_PATH = "./health_ai_model/final"
...
def compute_features(hr, gsr, temperature, spo2, age, past_encoded):
    ...
    hr_temp_ratio = hr / (temperature + 1e-6)
    oxygen_stress_index = (100 - spo2) + (gsr / 1000)
    temp_spo2_interaction = temperature * (100 - spo2)
    hrv_index = abs(hr - np.mean([hr])) / 10  # HRV cannot be computed with single reading
...
def get_disease_advice(disease, hr, gsr, temp, spo2, age):
    prompt = f"""
### Instruction:
You are a medical health assistant. Analyze the patient's data and predicted disease.

**Sensor Readings:**
- Heart Rate: {hr} BPM
- GSR (Skin Response): {gsr}
- Temperature: {temp}°C
- SpO₂: {spo2}%
- Age: {age}
...
Provide a *detailed, professional health analysis* including:
1. What these readings indicate
2. Why the condition may be happening
3. Risk factors
4. Immediate precautions
5. When emergency help is needed
6. Lifestyle advice

Keep explanation simple and easy for teenagers to understand.
...
def ai_loop():
    ...
    while True:
        try:
            ...
            advice = get_disease_advice(disease, hr, gsr, temperature, spo2, age)

            response = f"The predicted disease is {disease} {advice}"
...
def main_dash(master):
    ...
    spo2_status = condition_aqi(spo2_val)
    ...
    metrics_refs["Sp02"] = create_metric(
        main_frame, 1, ICON_AIR_QUALITY, "SpO2 level", spo2_val,
        UNIT_SPO2, spo2_status, status_to_color(spo2_status)
    )
    ...
    def update_live_metrics():
        ...
        metrics_refs["SpO2 Level"]["value"].configure(text=str(spo2))
        ...
        metrics_refs["SpO2 Level"]["badge"].configure(fg_color=status_to_color(condition_aqi(spo2)))
